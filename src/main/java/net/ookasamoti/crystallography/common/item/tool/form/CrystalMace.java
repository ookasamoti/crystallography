...
/**
 * ロードアウトのアクティブフォームが MACE（PICKAXE + heavy_core での昇格先）のときの実 Item。
 * 本物の {@link MaceItem} を継承するため、落下スマッシュ攻撃（{@code hurtEnemy}）は vanilla 実装を
 * そのまま継承して得られる。{@code hurtEnemy} は使用不可（破損／結晶欠落）ガードのみ追加し、
 * それ以外は必ず {@code super} へ委譲すること（スマッシュ判定自体を上書きしないため）。
 */
public class CrystalMace extends MaceItem implements ICrystalTool {

    private final int tier;

    public CrystalMace(Properties props, int tier) {
        super(props);
        this.tier = tier;
    }
