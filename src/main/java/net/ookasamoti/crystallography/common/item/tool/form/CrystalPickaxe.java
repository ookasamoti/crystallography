...
/**
 * ロードアウトのアクティブフォームが PICKAXE のときの実 Item。このバージョンの vanilla には
 * 専用の {@code PickaxeItem} クラスが存在しない（{@code Item.Properties#pickaxe} で DataComponent
 * のみ付与する形）ため、素の {@link Item} を継承する。
 */
public class CrystalPickaxe extends Item implements ICrystalTool {

    private final int tier;

    public CrystalPickaxe(Properties props, int tier) {
        super(props);
        this.tier = tier;
    }
