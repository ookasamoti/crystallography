...
/**
 * ロードアウトのアクティブフォームが FISHING_ROD のときの実 Item。本物の {@link FishingRodItem} を
 * 継承するため、浮き玉の投げ入れ・回収は vanilla 実装をそのまま継承して得られる。{@code use}
 * （投げ入れ）と {@code canPerformAction} は使用不可（破損／結晶欠落）ガードを追加する。
 */
public class CrystalFishingRod extends FishingRodItem implements ICrystalTool {

    private final int tier;

    public CrystalFishingRod(Properties props, int tier) {
        super(props);
        this.tier = tier;
    }
