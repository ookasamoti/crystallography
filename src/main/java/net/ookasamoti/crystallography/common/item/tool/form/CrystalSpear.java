...
/**
 * ロードアウトのアクティブフォームが SPEAR のときの実 Item。このバージョンの vanilla には専用の
 * {@code SpearItem} クラスは存在せず、素の {@link Item} に {@code KineticWeapon}/{@code PiercingWeapon}
 * 等の DataComponent を付与する形で表現される（{@code Item.Properties#spear}）。現時点では従来の
 * 近接攻撃のみ（vanilla 槍の突きチャージ演出は未実装）。結晶ステータスに応じたコンポーネント値の
 * 算出は今後の対応（design plan Phase 7）。
 */
public class CrystalSpear extends Item implements ICrystalTool {

    private final int tier;

    public CrystalSpear(Properties props, int tier) {
        super(props);
        this.tier = tier;
    }
