...
/**
 * ロードアウトのアクティブフォームが SWORD のときの実 Item。このバージョンの vanilla には専用の
 * {@code SwordItem} クラスが存在しない（純粋に DataComponent のみで表現される）ため、素の
 * {@link Item} を継承する。sweep 攻撃の有効化は {@link CrystalToolLogic#canPerformAction} が
 * {@code ItemAbilities.SWORD_SWEEP} を返すことで成立する。
 */
public class CrystalSword extends Item implements ICrystalTool {

    private final int tier;

    public CrystalSword(Properties props, int tier) {
        super(props);
        this.tier = tier;
    }
