...
/**
 * ロードアウトのアクティブフォームが TRIDENT（SPEAR + trident_core での昇格先）のときの実 Item。
 * 本物の {@link TridentItem} を継承するため、投擲（riptide/loyalty 含む）は vanilla 実装を
 * そのまま継承して得られる。{@code use}（投擲チャージ開始）と {@code canPerformAction} は
 * 使用不可（破損／結晶欠落）ガードを追加する。
 */
public class CrystalTrident extends TridentItem implements ICrystalTool {

    private final int tier;

    public CrystalTrident(Properties props, int tier) {
        super(props);
        this.tier = tier;
    }
