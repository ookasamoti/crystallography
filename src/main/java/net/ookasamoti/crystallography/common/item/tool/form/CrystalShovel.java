...
/**
 * ロードアウトのアクティブフォームが SHOVEL のときの実 Item。本物の {@link ShovelItem} を継承し、
 * 道の整地・焚き火消火は vanilla 実装をそのまま継承する。{@link ShovelItem} 自身はロードアウトの
 * 使用可否を見ないため、{@code useOn}/{@code canPerformAction} はここでガードを追加する。
 */
public class CrystalShovel extends ShovelItem implements ICrystalTool {

    private final int tier;

    public CrystalShovel(Properties props, int tier) {
        super(ToolMaterial.DIAMOND, ToolForm.SHOVEL.baseAttack(), ToolForm.SHOVEL.baseAttackSpeed(), props);
        this.tier = tier;
    }
