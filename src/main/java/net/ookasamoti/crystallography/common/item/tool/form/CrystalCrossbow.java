...
/**
 * ロードアウトのアクティブフォームが CROSSBOW のときの実 Item。本物の {@link CrossbowItem} を
 * 継承するため、装填・発射は vanilla 実装をそのまま継承して得られる。{@code use}（装填開始）は
 * 使用不可（破損／結晶欠落）ガードを追加する。
 */
public class CrystalCrossbow extends CrossbowItem implements ICrystalTool {

    private final int tier;

    public CrystalCrossbow(Properties props, int tier) {
        super(props);
        this.tier = tier;
    }
